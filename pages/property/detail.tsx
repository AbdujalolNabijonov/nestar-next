...
const PropertyDetail: NextPage = ({ initialComment, initialDestinations, ...props }: any) => {
	...
	const {
		...
		notifyOnNetworkStatusChange: true,
		variables: { input: { ...initialDestinations, search: { locationList: property?.propertyLocation } } },
		skip: !propertyId,
		onCompleted: (data: T) => {
	...
	/** HANDLERS **/
	const likeMemberPropertyHandle = async (user: any, propertyId: any) => {
		try {
			...
			await getPropertiesRefetch({ input: { ...initialDestinations, search: { locationList: property?.propertyLocation } } })
		} catch (err: any) {
			console.log("ERROR: likeMemberPropertyHandle,", err.message)
							...
							<Stack className={'info'}>
								...
								<Stack className={'right-box'}>
									<Stack className="buttons">
										...
										<Stack className="button-box" onClick={() => likeMemberPropertyHandle(user, property?._id)}>
